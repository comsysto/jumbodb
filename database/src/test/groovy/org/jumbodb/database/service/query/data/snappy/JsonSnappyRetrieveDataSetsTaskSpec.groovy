...
class JsonSnappyRetrieveDataSetsTaskSpec extends Specification {
    ...
    def createTestData() {
        ...
        for (i in 100000..101000) {
            def data = '{msg: "This is a sample dataset", number: ' + i + '}'  // 49 chars long + 4 length
    ...
    def "integrated test indexed search"() {
        ...
        when:
        def offsets = [530l, 1060l, 1590l].collect { new FileOffset(123, it, new IndexQuery()) } as Set
        ...
        when:
        offsets = [53000l].collect { new FileOffset(123, it, new IndexQuery()) } as Set
        task = new JsonSnappyRetrieveDataSetsTask(file, offsets, jumboQuery, resultCallback, dataStrategy, cacheMock, cacheMock, "yyyy-MM-dd", false)
        numberOfResults = task.call()
        ...
        when:
        offsets = [32754l].collect { new FileOffset(123, it, new IndexQuery()) } as Set
        task = new JsonSnappyRetrieveDataSetsTask(file, offsets, jumboQuery, resultCallback, dataStrategy, cacheMock, cacheMock, "yyyy-MM-dd", false)
        numberOfResults = task.call()
        then: "verify loading of a data set overlapping the snappy chunk (32768 byte)"
        1 * resultCallback.writeResult([msg: "This is a sample dataset", number: 100618])
        numberOfResults == 1
        cleanup:
