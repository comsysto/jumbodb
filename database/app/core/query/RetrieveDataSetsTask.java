...
public class RetrieveDataSetsTask implements Callable<Integer> {
    ...
    private final OlchingQuery searchQuery;
    ...
    public RetrieveDataSetsTask(File file, Set<Long> offsets, OlchingQuery searchQuery, ResultCallback resultCallback) {
        this.file = file;
    ...
    private boolean matchingFilter(String s, JSONParser parser) throws IOException, ParseException {
        ...
        for (OlchingQuery.JsonValueComparision jsonValueComparision : searchQuery.getJsonComparision()) {
            ...
            if (jsonValueComparision.getComparisionType() == OlchingQuery.JsonComparisionType.EQUALS &&
                    lastObj != null) {
                // CARSTEN vielleicht ist hier ein hashset schneller?
                matching &= jsonValueComparision.getValues().contains(lastObj);
            } else if (jsonValueComparision.getComparisionType() == OlchingQuery.JsonComparisionType.EQUALS_IGNORE_CASE) {
