 ...
 * @author Carsten Hufe
 */
class HashCode32Lz4IndexStrategySpec extends Specification {
    def strategy = new HashCode32Lz4IndexStrategy()
    ...
    def "searchOffsetsByIndexQueries"() {
        ...
        def strategy = new HashCode32Lz4IndexStrategy()
        setupCache(strategy)
        def indexFile = HashCode32Lz4DataGeneration.createFile()
        HashCode32Lz4DataGeneration.createIndexFile(indexFile)
    ...
    def "findOffsetForIndexQuery"() {
        ...
        setupCache(strategy)
        def indexFile = HashCode32Lz4DataGeneration.createFile()
        def blocks = HashCode32Lz4DataGeneration.createIndexFile(indexFile)
    ...
    def "createIndexFileDescription"() {
        setup:
        def indexFile = HashCode32Lz4DataGeneration.createFile()
        def lz4 = HashCode32Lz4DataGeneration.createIndexFile(indexFile)
        when:
        def indexFileDescription = strategy.createIndexFileDescription(indexFile, lz4)
        then:
    ...
    def createIndexFolder() {
        ...
        indexFolder.mkdirs()
        HashCode32Lz4DataGeneration.createIndexFile(new File(indexFolder.getAbsolutePath() + "/part00001.idx"))
        indexFolder
    }

    def createCollectionDefinition(indexFolder) {
        def index = new IndexDefinition("testIndex", indexFolder, "HASHCODE32_LZ4")
        def cdMap = [testCollection: [new DeliveryChunkDefinition("testChunkKey", "testCollection", "yyyy-MM-dd", [index], [:], "HASHCODE32_LZ4")]]
