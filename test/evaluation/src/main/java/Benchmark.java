...
//        fos.close();
    ...
    private static void lz4decompress() throws IOException {
        long start = System.currentTimeMillis();
        FileInputStream fis = new FileInputStream("c:/Development/sample-0002.json.lz4");
        BufferedInputStream bis = new BufferedInputStream(fis);
        LZ4BlockInputStream lz4In = new LZ4BlockInputStream(bis);
//        FileOutputStream fos = new FileOutputStream("c:/Development/sample-0002.json.lz4.dec");
//        BufferedOutputStream bos = new BufferedOutputStream(fos);
        OutputStream bos = new NullOutputStream();
        IOUtils.copy(lz4In, bos);
        lz4In.close();
