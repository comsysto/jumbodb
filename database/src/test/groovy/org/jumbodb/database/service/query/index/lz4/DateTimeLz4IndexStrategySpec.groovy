 ...
 * @author Carsten Hufe
 */
class DateTimeLz4IndexStrategySpec extends Specification {
    def strategy = new DateTimeLz4IndexStrategy()
    ...
    def "searchOffsetsByIndexQueries"() {
        ...
        setup:
        def strategy = new DateTimeLz4IndexStrategy()
        def indexFile = DateTimeLz4DataGeneration.createFile()
        DateTimeLz4DataGeneration.createIndexFile(indexFile)
        def cacheMock = Mock(Cache)
        cacheMock.get(_) >> null
        strategy.setIndexCompressionBlocksCache(cacheMock)
        strategy.setIndexBlockRangesCache(cacheMock)
    ...
    def "findOffsetForIndexQuery"() {
        ...
        def strategy = new DateTimeLz4IndexStrategy()
        def indexFile = DateTimeLz4DataGeneration.createFile()
        def blocks = DateTimeLz4DataGeneration.createIndexFile(indexFile)
    ...
    def createIndexFolder() {
        ...
        indexFolder.mkdirs()
        DateTimeLz4DataGeneration.createIndexFile(new File(indexFolder.getAbsolutePath() + "/part00001.idx"))
        indexFolder
    }

    def createCollectionDefinition(indexFolder) {
        def index = new IndexDefinition("testIndex", indexFolder, "DATETIME_LZ4")
        def cdMap = [testCollection: [new DeliveryChunkDefinition("testChunkKey", "testCollection", "yyyy-MM-dd", [index], [:], "DATETIME_LZ4")]]
