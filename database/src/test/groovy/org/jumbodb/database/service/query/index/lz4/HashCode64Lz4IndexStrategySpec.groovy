 ...
 * @author Carsten Hufe
 */
class HashCode64Lz4IndexStrategySpec extends Specification {
    def strategy = new HashCode64Lz4IndexStrategy()
    ...
    def "searchOffsetsByIndexQueries"() {
        ...
        def strategy = new HashCode64Lz4IndexStrategy()
        setupCache(strategy)
        def indexFile = HashCode64Lz4DataGeneration.createFile()
        HashCode64Lz4DataGeneration.createIndexFile(indexFile)
    ...
    def "findOffsetForIndexQuery"() {
        ...
        setupCache(strategy)
        def indexFile = HashCode64Lz4DataGeneration.createFile()
        def blocks = HashCode64Lz4DataGeneration.createIndexFile(indexFile)
    ...
    def createIndexFolder() {
        ...
        indexFolder.mkdirs()
        HashCode64Lz4DataGeneration.createIndexFile(new File(indexFolder.getAbsolutePath() + "/part00001.idx"))
        indexFolder
    }

    def createCollectionDefinition(indexFolder) {
        def index = new IndexDefinition("testIndex", indexFolder, "HASHCODE64_LZ4")
        def cdMap = [testCollection: [new DeliveryChunkDefinition("testChunkKey", "testCollection", "yyyy-MM-dd", [index], [:], "HASHCODE64_LZ4")]]
