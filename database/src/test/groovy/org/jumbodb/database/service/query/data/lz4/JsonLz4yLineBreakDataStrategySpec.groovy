 ...
 * @author Carsten Hufe
 */
class JsonLz4yLineBreakDataStrategySpec extends Specification {
    def strategy = new JsonLz4LineBreakDataStrategy()
    ...
    def "should be responsible, because collection, chunk and strategy are matching"() {
        ...
        def dcd = new DeliveryChunkDefinition("chunk", "collection", "yyyy-MM-dd", [], [:], "JSON_LZ4_LB")
    ...
    def "verify strategy name"() {
        expect:
        strategy.getStrategyName() == "JSON_LZ4_LB"
    }
    ...
    def "matches should delegate to the appropriate operation"() {
        ...
        def customStrategy = new JsonLz4LineBreakDataStrategy() {
            @Override
    ...
    def "findDataSetsByFileOffsets should run index search and submit 2 tasks, because offsets are spread in 2 files"() {
        ...
        File mockFile = Mock()
        def indexes = [new IndexDefinition("myindex", mockFile, JsonLz4LineBreakDataStrategy.JSON_LZ4_LB)]
        def dataFiles = new HashMap<Integer, File>();
        dataFiles.put(1, mockFile)
        dataFiles.put(2, mockFile)
        dataFiles.put(3, mockFile)
        def deliveryChunkDefinition = new DeliveryChunkDefinition("testchunkkey", "testcollection", "yyyy-MM-dd", indexes, dataFiles, JsonLz4LineBreakDataStrategy.JSON_LZ4_LB)
        def fileOffsets = [
                new FileOffset(1, 12, null),
                new FileOffset(1, 13, null),
                new FileOffset(2, 15, null),
        ...
        then:
        numberOfResults == 6
        2 * executorService.submit(_ as JsonLz4LineBreakRetrieveDataSetsTask) >> futureMock
        2 * futureMock.get() >> 3
    }
    ...
    def "findDataSetsByFileOffsets should run scanned search and submit 3 tasks, because no index and 3 files are existent"() {
        ...
        dataFiles.put(2, mockFile)
        dataFiles.put(3, mockFile)
        def deliveryChunkDefinition = new DeliveryChunkDefinition("testchunkkey", "testcollection", "yyyy-MM-dd", [], dataFiles, JsonLz4LineBreakDataStrategy.JSON_LZ4_LB)
        def jumboQuery = new JumboQuery()
        def resultCallback = Mock(ResultCallback)
        when:
        ...
        then:
        numberOfResults == 9
        3 * executorService.submit(_ as JsonLz4LineBreakRetrieveDataSetsTask) >> futureMock
        3 * futureMock.get() >> 3
    }
    ...
    def "getCollectionDataSize"() {
        ...
        Lz4Util.copy(new ByteArrayInputStream(bytes), dataFile, bytes.length, 100l, 32 * 1024)
        def strategy = new JsonLz4LineBreakDataStrategy()
        when:
