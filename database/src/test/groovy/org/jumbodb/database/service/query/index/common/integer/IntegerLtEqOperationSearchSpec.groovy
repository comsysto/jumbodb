...
class IntegerLtEqOperationSearchSpec extends Specification {
    ...
    def "findFirstMatchingBlock #searchValue with expected chunk #expectedChunk"() {
        ...
        expect:
        operation.findFirstMatchingBlock(retriever, operation.getQueryValueRetriever(new IndexQuery("testIndex", QueryOperation.LT_EQ, searchValue)), blocks) == expectedBlock
        cleanup:
//        ramFile.close()
        file.delete();
        where:
