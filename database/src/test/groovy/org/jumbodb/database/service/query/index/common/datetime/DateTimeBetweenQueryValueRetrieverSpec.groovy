...
import org.jumbodb.database.service.query.index.common.datetime.DateTimeBetweenQueryValueRetriever
...
class DateTimeBetweenQueryValueRetrieverSpec extends spock.lang.Specification {
    ...
    def "verify date string parsing #queryValue"() {
        expect:
        def retriever = new DateTimeBetweenQueryValueRetriever(new IndexQuery("testIndex", QueryOperation.BETWEEN, queryValue))
    ...
    def "expect exception on bullshit string"() {
        when:
        new DateTimeBetweenQueryValueRetriever(new IndexQuery("testIndex", QueryOperation.BETWEEN, ["bullshit", "andso"]))
        then:
