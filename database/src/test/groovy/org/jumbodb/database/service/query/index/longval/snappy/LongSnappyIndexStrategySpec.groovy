...
class LongSnappyIndexStrategySpec extends Specification {
    ...
    def "onImport"() {
        setup:
        def indexFileContent = LongDataGeneration.createIndexContent()
        ...
        md5hash == "8e30fdf7186b5d460a0ffe26b0d78cc7"
        cleanup:
        indexFolder.delete()
    ...
    def createIndexFolder() {
        def indexFolder = new File(System.getProperty("java.io.tmpdir") + "/" + UUID.randomUUID().toString() + "/")
        indexFolder.mkdirs()
        LongDataGeneration.createIndexFile(new File(indexFolder.getAbsolutePath() + "/part00001.odx"))
        indexFolder
    }

    def createCollectionDefinition(indexFolder) {
        def index = new IndexDefinition("testIndex", indexFolder, "LONG_SNAPPY_V1")
        def cdMap = [testCollection: [new DeliveryChunkDefinition("testCollection", "testChunkKey", [index], [:], "LONG_SNAPPY_V1")]]
