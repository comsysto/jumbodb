...
class HashCode32SnappyIndexStrategySpec extends Specification {
    ...
    def "onImport"() {
        setup:
        def indexFileContent = HashCode32DataGeneration.createIndexContent()
    ...
    def createIndexFolder() {
        def indexFolder = new File(System.getProperty("java.io.tmpdir") + "/" + UUID.randomUUID().toString() + "/")
        indexFolder.mkdirs()
        HashCode32DataGeneration.createIndexFile(new File(indexFolder.getAbsolutePath() + "/part00001.odx"))
        indexFolder
    }
