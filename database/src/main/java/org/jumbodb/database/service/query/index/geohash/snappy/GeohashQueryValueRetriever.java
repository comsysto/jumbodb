...
public class GeohashQueryValueRetriever implements QueryValueRetriever {
    ...
    private static final BoundingBox upperLeft = new BoundingBox(new WGS84Point(0d, -180d),
      new WGS84Point(90d, -0.0000001d));
    private static final BoundingBox upperRight = new BoundingBox(new WGS84Point(0d, 0d), new WGS84Point(90d, 180d));
    private static final BoundingBox bottomLeft = new BoundingBox(new WGS84Point(-90d, -180d),
      new WGS84Point(-0.0000001d, -0.0000001d));
    private static final BoundingBox bottomRight = new BoundingBox(new WGS84Point(-90d, 0d),
      new WGS84Point(-0.0000001d, 180d));
    ...
    public GeohashQueryValueRetriever(IndexQuery indexQuery) {
        ...
        if (indexQuery.getQueryOperation() == QueryOperation.GEO_WITHIN_RANGE_METER) {
            ...
            initializeBoundaryBox(Math.min(lat1, lat2), Math.min(lon1, lon2), Math.max(lat1, lat2),
              Math.max(lon1, lon2));
        }
    }
    ...
    private void initializeBoundaryBox(double latitude1, double longitude1, double latitude2, double longitude2) {
        ...
        if (upperLeft.intersects(boundingBox)) {
            double minLat = upperLeft.getMinLat() > boundingBox.getMinLat() ? upperLeft.getMinLat() : boundingBox
              .getMinLat();
            double minLon = boundingBox.getMinLon();  // bleibt
            double maxLat = boundingBox.getMaxLat();  // bleibt
            double maxLon = upperLeft.getMaxLon() < boundingBox.getMaxLon() ? upperLeft.getMaxLon() : boundingBox
              .getMaxLon();
            value.add(
              createGeohashBoundaryBox(new BoundingBox(new WGS84Point(minLat, minLon), new WGS84Point(maxLat, maxLon)),
                upperLeft));
        }
        //        else {
        ...
        if (upperRight.intersects(boundingBox)) {
            double minLat = upperRight.getMinLat() > boundingBox.getMinLat() ? upperRight.getMinLat() : boundingBox
              .getMinLat();
            double minLon = upperRight.getMinLon() > boundingBox.getMinLon() ? upperRight.getMinLon() : boundingBox
              .getMinLon();
            double maxLat = boundingBox.getMaxLat();  // bleibt
        ...
        if (bottomLeft.intersects(boundingBox)) {
            ...
            double maxLat = bottomLeft.getMaxLat() < boundingBox.getMaxLat() ? bottomLeft.getMaxLat() : boundingBox
              .getMaxLat();
            double maxLon = bottomLeft.getMaxLon() < boundingBox.getMaxLon() ? bottomLeft.getMaxLon() : boundingBox
              .getMaxLon();
            value.add(
              createGeohashBoundaryBox(new BoundingBox(new WGS84Point(minLat, minLon), new WGS84Point(maxLat, maxLon)),
                bottomLeft));
        ...
        if (bottomRight.intersects(boundingBox)) {
            ...
            double minLon = bottomRight.getMinLon() > boundingBox.getMinLon() ? bottomRight.getMinLon() : boundingBox
              .getMinLon();
            double maxLat = bottomRight.getMaxLat() < boundingBox.getMaxLat() ? bottomRight.getMaxLat() : boundingBox
              .getMaxLat();
            double maxLon = boundingBox.getMaxLon(); // bleibt
            value.add(
              createGeohashBoundaryBox(new BoundingBox(new WGS84Point(minLat, minLon), new WGS84Point(maxLat, maxLon)),
