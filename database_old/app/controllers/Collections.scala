...
object Collections extends Controller {
  ...
  val Home = Redirect(routes.Collections.overview())
  ...
  def deleteCompleteCollection(collection: String) = Action {
    Storage.management.deleteCompleteCollection(collection)
    Home.flashing("delete" -> ("Complete collection '" + collection + "' has been deleted."))
  }
