...
class JsonSnappyRetrieveDataSetsTaskSpec extends spock.lang.Specification {
    ...
    def "matchingFilter"() {
        ...
        task.matchingFilter('{"sample": "json"}', jsonParser)
        ...
        def matching = task.matchingFilter('{"sample": "json", "otherfield": "othervalue"}', jsonParser)
        ...
        jumboQuery.addJsonQuery("otherfield", [new QueryClause(QueryOperation.EQ, 'othervalue')])
        task = new JsonSnappyRetrieveDataSetsTask(file, [] as Set, jumboQuery, resultCallback, dataStrategy)
        def notMatching = !task.matchingFilter('{"sample": "json", "otherfield": "otherNEvalue"}', jsonParser)
        ...
        task = new JsonSnappyRetrieveDataSetsTask(file, [] as Set, jumboQuery, resultCallback, dataStrategy)
        matching = task.matchingFilter('{"sample": "json", "otherfield": "otherNEvalue"}', jsonParser)
        ...
        task = new JsonSnappyRetrieveDataSetsTask(file, [] as Set, jumboQuery, resultCallback, dataStrategy)
        notMatching = !task.matchingFilter('{"sample": "json", "otherfield": "otherNEvalue"}', jsonParser)
    ...
    def "getBufferByOffsetGroup(#offsetGroup) == #expectedBufferSize"() {
        ...
        def task = createDefaultTask()
        expect:
        task.getBufferByOffsetGroup(offsetGroup).length == expectedBufferSize
        where:
    ...
    def "groupOffsetsByBufferSize == #expectedGroups"() {
        ...
        def task = createDefaultTask()
        expect:
        task.groupOffsetsByBufferSize(offsets, bufferSize) == expectedGroups
        where:
    ...
    def "groupOffsetsByBufferSize test maximum offset group"() {
        ...
        def task = createDefaultTask()
        when:
        def offsets = (1..2500).collect { Long.valueOf(it * 500) }
