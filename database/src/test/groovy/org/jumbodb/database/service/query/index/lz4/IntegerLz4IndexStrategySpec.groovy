 ...
 * @author Carsten Hufe
 */
class IntegerLz4IndexStrategySpec extends Specification {
    ...
    def "findOffsetForIndexQuery"() {
        ...
        def fileOffsets = strategy.findOffsetForIndexQuery(indexFile, ramFile, indexQuery, lz4Blocks, 5, true)
        then:
        ...
        fileOffsets = strategy.findOffsetForIndexQuery(indexFile, ramFile, indexQuery, lz4Blocks, 5, true)
        then:
        fileOffsets.size() == 0
        cleanup:
        ramFile.close()
    ...
    def createCollectionDefinition(indexFolder) {
        def index = new IndexDefinition("testIndex", indexFolder, "INTEGER_LZ4")
        def cdMap = [testCollection: [new DeliveryChunkDefinition("testChunkKey", "testCollection", "yyyy-MM-dd", [index], [:], "INTEGER_LZ4")]]
